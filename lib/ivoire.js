...
(function() {
  var Ivoire, MAX_INT, MIN_INT, MersenneTwister, assert, panic_if;
  ...
  panic_if = function(test, errorMessage) {
    return assert(!test, errorMessage);
  };
  ...
  Ivoire = (function() {
    ...
    Ivoire.prototype.panic_if = panic_if;

    Ivoire.prototype.assert = assert;
    ...
    Ivoire.prototype.integer = function(options) {
      var max, min, rand;
      min = options && options.min === void 0 ? MIN_INT : options.min;
      max = options && options.max === void 0 ? MAX_INT : options.max;
      this.panic_if(min > max, "Min cannot be greater than Max.");
      rand = Math.floor(this.random() * (max - min + 1)) + min;
      return rand;
    };
